...
import transformers
import torch
from transformers import CONFIG_MAPPING, MODEL_WITH_LM_HEAD_MAPPING, AutoConfig, AutoModelWithLMHead, \
    DataCollatorForLanguageModeling, DataCollatorForPermutationLanguageModeling, HfArgumentParser, \
        LineByLineTextDataset, PreTrainedTokenizer, TextDataset, XLNetLMHeadModel, set_seed

from terminator.tokenization import ExpressionBertTokenizer
from terminator.trainer import CustomTrainer, get_trainer_dict
from terminator.args import ModelArguments, CustomTrainingArguments
...
class DataTrainingArguments:
    ...
    eval_data_file: Optional[str] = field(
        default=None, metadata={"help": "Input evaluation data file to evaluate the perplexity on (a text file)."})
    line_by_line: bool = field(
        default=False, metadata={"help": "Whether lines of text in the dataset are to be handled as distinct samples."})

    mlm: bool = field(default=False, metadata={"help": "Train with masked-LM loss instead of LM"})
...
def main():
    ...
    logger.warning("Process rank: %s, device: %s, n_gpu: %s, distributed training: %s", training_args.local_rank,
                   training_args.device, training_args.n_gpu, bool(training_args.local_rank != -1))
    ...
    if model_args.tokenizer_name:
        tokenizer = ExpressionBertTokenizer.from_pretrained(model_args.tokenizer_name, cache_dir=model_args.cache_dir)

    elif model_args.model_name_or_path:
        tokenizer = ExpressionBertTokenizer.from_pretrained(model_args.model_name_or_path,
                                                            cache_dir=model_args.cache_dir)
    else:
        raise ValueError(
    ...
    if model_args.model_name_or_path:
        ...
        if "checkpoint" not in model_args.model_name_or_path:
            model_args.model_name_or_path = get_latest_checkpoint(model_args.model_name_or_path)
        ...
        try:
            ...
            logger.warning("Could not find loss history, might overwrite good checkpoints.")
    ...
    # Get datasets
    train_dataset = (get_dataset(data_args.train_data_file, tokenizer=tokenizer, block_size=data_args.block_size)
                     if training_args.do_train else None)
    eval_dataset = (get_dataset(data_args.eval_data_file, tokenizer=tokenizer, block_size=data_args.block_size,
                                line_by_line=data_args.line_by_line) if training_args.do_eval else None)
    ...
    if training_args.training_config_path is not None:
        ...
        with open(os.path.join(training_args.output_dir, "training_configs.json"), "w") as f:
    ...
    if train_config.get("alternate_tasks", False):
        ...
        alternating_collator = TRAIN_COLLATORS[train_config["cg_collator"]](tokenizer=tokenizer,
                                                                            **train_config["cg_collator_params"])
    ...
    trainer = CustomTrainer(model=model, args=training_args, data_collator=data_collator, train_dataset=train_dataset,
                            eval_dataset=eval_dataset, tokenizer=tokenizer, prediction_loss_only=False,
                            alternating_collator=alternating_collator, train_config=train_config,
